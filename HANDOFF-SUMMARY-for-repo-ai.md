...
- **Guest-facing phone country code default (+1) and formatting** — client wants this on the GUEST'S OWN registration/pre-checkin phone field (separate from the admin form, which was already fixed). Need to locate the guest-facing phone input in `show.blade.php` and confirm whether it already has a country-code default or needs one added, and apply the same `(000) 123-4567` live-format mask pattern already used on the admin form.
